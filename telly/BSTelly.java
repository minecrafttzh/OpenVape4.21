/*
by NeverBeBanned, open source to everyone❤
bilibili:h473-u
*/
boolean armed = false;
...
void drawActivationFaceRegion(int[] pos, int face) {
    ...
    gl.depth(false);
    gl.depthMask(false);
    gl.translate((float)-cam.x, (float)-cam.y, (float)-cam.z);

    gl.color(r, g, b, fillAlpha);
    gl.begin(7);
    gl.vertex3((float)x1, (float)yMin, (float)z1);
    gl.vertex3((float)x2, (float)yMin, (float)z2);
    gl.vertex3((float)x2, (float)yMax, (float)z2);
    gl.vertex3((float)x1, (float)yMax, (float)z1);
    gl.end();
...
void drawActivatePrompt() {
    ...
    render.text2d(text, x, y, 1.0f, color, true);
}
...
boolean attemptPlacement(Entity player, Object[] candidate, ItemStack heldStack) {
    ...
    client.swing();  // 修复: 使用 swing() 替代不存在的 swingReset()
    return true;
}
...
Object[] findBelowPlacement(Entity player, float yaw, float currentPitch, ItemStack heldStack, long deadlineMs) {
    ...
    for (int dy = 0; dy <= 2; dy++) {
        ...
        if (targetY == -2147483648 || (dy == 1 && targetY == currentY) || (dy == 2 && (targetY == currentY || targetY == strictY))) continue;
