...
public class RuntimeNameMappingRegistry {
    ...
    public static String remapClassName(String sourceClassName) {
        ...
        boolean mojmapRuntime = NativeBridge.isNeoForge1201Runtime()
                ...
                || NativeBridge.isForge1201Runtime()
                || NativeBridge.isForge1211Runtime()
                // Forge 1.20.1/1.21.1 runs mojmap class names (with SRG member
                // names), so its class names must resolve like a mojmap runtime.
        ...
        if (mapped != null) {
            if (mojmapRuntime) {
                ...
                boolean use1211 = NativeBridge.isNeoForge1211Runtime()
                        || NativeBridge.isForge1211Runtime()
                        || (!NativeBridge.isForgeAbsent()
                                && ForgeVersion.c() == 52);
                String mojmap = use1211
        ...
        boolean forge1201Mojmap = NativeBridge.isForge1201Runtime()
                || (!NativeBridge.isForgeAbsent() && ForgeVersion.c() == 47);
        boolean forge1211Mojmap = NativeBridge.isForge1211Runtime()
                || (!NativeBridge.isForgeAbsent() && ForgeVersion.c() == 52);
        if (NativeBridge.isNeoForge1211Runtime() || forge1211Mojmap) {
