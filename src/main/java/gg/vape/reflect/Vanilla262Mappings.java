...
public final class Vanilla262Mappings {
    ...
    public static boolean isRuntimePresent(ClassLoader... preferredLoaders) {
        return protocolVersion(preferredLoaders) != 0;
    }

    /**
     * Returns the Vape-encoded protocol number (100 for 26.1.x, 110 for 26.2.x)
    ...
    public static int protocolVersion(ClassLoader... preferredLoaders) {
        for (ClassLoader loader : candidateLoaders(preferredLoaders)) {
            ...
            if (matchesMinecraftStructure(minecraftClass)) {
                int protocol = matchesVersionMetadataProtocol(minecraftClass);
                if (protocol != 0) {
                    return protocol;
                }
            }
    ...
    private static boolean matchesVersionMetadata(Class<?> minecraftClass) {
        return matchesVersionMetadataProtocol(minecraftClass) == 110;
    }

    private static int matchesVersionMetadataProtocol(Class<?> minecraftClass) {
