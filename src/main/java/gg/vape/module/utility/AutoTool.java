...
extends Mod {
    ...
    // 不在战斗时才换工具：参考 InventoryManager 的 Combat check / InventoryActionGuard。
    ...
    // 不在使用物品时才换工具：喝药水、吃食物、格挡、拉弓时换手会打断使用，因此默认关。
    ...
    private final InventoryActionGuard combatGuard = new InventoryActionGuard(20);
    ...
    public void onTick(EventPrePlayerTick eventPrePlayerTick) {
        ...
        EntityPlayer player = eventPrePlayerTick.getPlayer();
        // 战斗检测：复用 InventoryActionGuard，与 InventoryManager.Combat check 行为一致。
        if (this.notDuringCombat.getEffectiveValue().booleanValue()) {
            this.combatGuard.update(player);
            if (this.combatGuard.isBlocked()) {
                return;
            }
        }
        // 使用物品检测：1.8.9 的 isUsingItem() 涵盖吃食物、喝药水、拉弓、格挡。
        // 切换槽位会中断使用动作，所以开启后在使用中禁止切换。
        if (this.notDuringUse.getEffectiveValue().booleanValue()
                && player.l$src$Z$1io4duf()) {
            return;
        }
