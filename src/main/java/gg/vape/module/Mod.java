...
EventListener {
    ...
    public void showToggleNotification() {
        ...
        // 开启时模块名使用白色 §f，状态文字仍为绿色 §a；关闭时全部保持红色 §c。
        if (this.isEnabled()) {
            this.toggleNotification.withTitle("§f" + this.getName())
                    .withMessage("§aEnabled").reset();
        } else {
            this.toggleNotification.withTitle("§c" + this.getName())
                    .withMessage("§cDisabled").reset();
        }
    ...
    public void setEnabled(boolean enabled, boolean bypassVisibilityCheck) {
        ...
        if (stateChanged) {
            ModuleToggleSoundTracker.recordStateChange(this.enabled);
        }
