...
public class Vape {
    ...
    public void initializeManagers() {
        ...
        // 注入后自动关闭处于 AI 模式的 SilentAura：AI 旋转依赖外部 MLP 模型，
        // 在恢复环境中默认不启用，避免注入后 SilentAura 以 AI 模式意外运行。
        try {
            SilentAura silentAura = this.modManager.getMod(SilentAura.class);
            if (silentAura != null && silentAura.isEnabled() && silentAura.isAiRotationMode()) {
